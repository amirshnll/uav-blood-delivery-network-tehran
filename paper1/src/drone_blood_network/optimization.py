...
def solve_set_cover(
    ...
    coverage = distances[
        distances["distance_km"] <= settings.max_operation_radius_km
    ].copy()
    ...
    if uncovered:
        raise RuntimeError(
            f"{len(uncovered)} demand points are not coverable with radius {settings.max_operation_radius_km} km "
            f"using candidate pool '{pool_label}'."
        )
