...
def run_validation(settings: Settings = DEFAULT_SETTINGS):
    ...
    temperature_summary_df, temperature_station_df = temperature_scenario_analysis(
        facilities, settings
    )
    save_validation_outputs(
        constraints_df,
...
def run_temperature_analysis(settings: Settings = DEFAULT_SETTINGS):
    facilities = load_facilities(settings)
    temperature_summary_df, temperature_station_df = temperature_scenario_analysis(
        facilities, settings
    )
    ...
    temperature_summary_df.to_csv(
        settings.results_dir / "validation_temperature_scenarios.csv", index=False
    )
    temperature_station_df.to_csv(
        settings.results_dir / "validation_temperature_station_details.csv",
        index=False,
    )
    return temperature_summary_df, temperature_station_df
...
def run_all(settings: Settings = DEFAULT_SETTINGS) -> dict:
    ...
    summary = {
        ...
        "temperature_scenario_count": int(len(temperature_summary_df)),
        "temperature_station_count_min": int(
            temperature_summary_df["station_count"].min()
        ),
        "temperature_station_count_max": int(
            temperature_summary_df["station_count"].max()
        ),
    }
