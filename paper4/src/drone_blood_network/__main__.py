...
def main() -> None:
            ...
            json.dumps(
                {
                    ...
                    "temperature_scenarios": len(temperature_summary_df),
                },
                indent=2,
            )
        )
    ...
    elif args.command == "temperature-analysis":
        temperature_summary_df, temperature_station_df = run_temperature_analysis()
        print(
            json.dumps(
                {
                    "temperature_scenarios": len(temperature_summary_df),
                    "station_detail_rows": len(temperature_station_df),
                    "station_count_min": int(temperature_summary_df["station_count"].min()),
                    "station_count_max": int(temperature_summary_df["station_count"].max()),
                },
                indent=2,
            )
        )
