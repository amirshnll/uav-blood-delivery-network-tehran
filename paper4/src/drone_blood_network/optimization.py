...
from .temperature import (
    ...
    temperature_distance_multiplier,
)
...
def solve_set_cover(
    ...
    temperature_multiplier = temperature_distance_multiplier(settings)
    performance_factor = battery_performance_factor(settings)
    ...
    distances["wind_distance_multiplier"] = wind_adjustments.apply(lambda value: value[0])
    distances["wind_alignment"] = wind_adjustments.apply(lambda value: value[1])
    distances["temperature_distance_multiplier"] = temperature_multiplier
    distances["battery_performance_factor"] = performance_factor
    distances["distance_multiplier"] = (
        distances["wind_distance_multiplier"]
        * distances["temperature_distance_multiplier"]
    )
        ...
        assignment_rows.append(
            {
                ...
                "wind_distance_multiplier": nearest["wind_distance_multiplier"],
                "temperature_distance_multiplier": nearest[
                    "temperature_distance_multiplier"
                ],
                "battery_performance_factor": nearest["battery_performance_factor"],
            }
        )
    ...
    assignment_df["temperature_scenario"] = temperature_scenario.code
    assignment_df["temperature_status_label"] = temperature_scenario.status_label
    assignment_df["temperature_range_label"] = temperature_scenario.ambient_range_label
    assignment_df["battery_performance_factor"] = performance_factor
    assignment_df["effective_operation_radius_km"] = effective_operation_radius_km
...
def save_optimization_outputs(result: OptimizationResult, output_dir: Path) -> None:
    ...
    temperature_scenario = (
        str(result.assignments["temperature_scenario"].iloc[0])
        if not result.assignments.empty
        and "temperature_scenario" in result.assignments
        else "S1"
    )
    battery_factor = (
        float(result.assignments["battery_performance_factor"].iloc[0])
        if not result.assignments.empty
        and "battery_performance_factor" in result.assignments
        else 1.0
    )
    effective_operation_radius_km = (
        float(result.assignments["effective_operation_radius_km"].iloc[0])
        if not result.assignments.empty
        and "effective_operation_radius_km" in result.assignments
        else 0.0
    )
    pd.DataFrame(
        [
            {
                ...
                "battery_performance_factor": battery_factor,
                "effective_operation_radius_km": effective_operation_radius_km,
            }
        ]
